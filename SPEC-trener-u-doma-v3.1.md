# Техническая спецификация v3.1 · Лендинг «Тренер у дома»
...
**Что нового в v3.1 (правки после ретро спринта 2):**
1. **Иконки — Lucide** (`lucide-react`), свои SVG выкидываем (§5.5).
2. **Кнопки — чистый cyan**, система размеров lg/md/sm на токенах (§5.4).
3. **Анимации обязательны для прода:** IntersectionObserver + плавный easing + reduced-motion (§5.6).
4. **Единая ширина 1080px** для всех секций; мостики — плавный скролл до якоря (§5.7).

**Открытые вопросы (решаются позднее, НЕ блокируют спринт 3):**
- Логотип — направление «монолайн чайка над волной», форма не финализирована.
- Карта локации — выбор подхода (тайлы Яндекса / SVG-схема / Static Maps) открыт; пока остаётся текущая premium-схема из `blocks-location-slot.html`.

**Что было в v3.0:** сведение в один документ; премиум-дизайн как часть DoD; секции карта локации и слот записи (честный MVP).

**Живые референсы (открыть перед версткой, копировать оттуда — НЕ воображать по тексту):**
- `BRAND-BOOK.html` — свод правил бренда (палитра, типографика, кнопки, голос, движение). Источник истины по стилю.
- `VISUAL-REFERENCE.html` — премиум-приёмы (фон, стекло, hairline, glow) с комментариями в CSS.
- `redesign-sprint3.html` — иконки, система кнопок, единая ширина, плавные анимации вживую.
...
- **Вторичная (secondary):** стекло `rgba(255,255,255,.02)` + бордер `--line` + `--edge-highlight`.
...
### 5.7. Ширина и ритм (v3.1)
- **Единый контейнер `--container:1080px`** для ВСЕХ секций. Узкие элементы (слот, лиды) центрируются внутри — рамка контента одна, края не «гуляют» (баг спринта 2 устранён).
- Между секциями — `--space-section:64px`; мостик прижат к своему блоку (анонсирует его).
- **Мостики — плавный скролл до якоря** следующего блока: `html{ scroll-behavior:smooth }` + якорные `<a href="#...">`. Отключается при reduced-motion.
